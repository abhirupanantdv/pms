...
const DynamicFormField = ({ field, value, onChange, linkOptionsCache, fetchLinkOptions, getDocTypeFields, erpnextConfig, getCsrfToken, formValues = {}, isNew = false, bookingPropertyGroup }) => {
  ...
  switch (fieldtype) {
    ...
    case 'Table':
      ...
      if (bookingPropertyGroup !== undefined && isOnboardingUnitTable(field)) {
        if (loadingChild) return <div style={{ gridColumn: 'span 2', padding: 12 }}>Loading onboarding units...</div>;
        return <BookingSelectedUnits
          field={field}
          fields={childFields}
          value={value}
          onChange={onChange}
          propertyGroup={bookingPropertyGroup}
          location={formValues.shop_space_location || ''}
...
export default function TenantOnboarding({ erpnextConfig, getCsrfToken }) {
  ...
  const isPreviewCompanySearchDoc = previewDocTitle &&
    previewDocTitle !== 'Signed Documents' &&
  ...
  const [bookingPropertyGroup, setBookingPropertyGroup] = useState('');
  const signedDocumentFieldname = doctypeFields.find(isSignedDocumentField)?.fieldname || 'signed_document';
  const bookingPropertyGroupField = doctypeFields.find(field => field.fieldtype === 'Link' && field.options === 'Property Group');
  const selectedBookingPropertyGroup = bookingPropertyGroupField
    ? (dynamicFormValues[bookingPropertyGroupField.fieldname] || '') : bookingPropertyGroup;
  const updateBookingSelection = (fieldname, value) => {
    setBookingPropertyGroup(fieldname === 'shop_space_location' ? '' : value);
    setDynamicFormValues(prev => {
      const next = fieldname ? { ...prev, [fieldname]: value } : { ...prev };
      if (fieldname === 'shop_space_location' && bookingPropertyGroupField) next[bookingPropertyGroupField.fieldname] = '';
      doctypeFields.filter(isOnboardingUnitTable).forEach(field => { next[field.fieldname] = []; });
      next.service_promotional_charges = 0;
      next.rental_charges = 0;
      next.security_deposit_booking_fee = 0;
      return next;
    });
  };
  ...
  const handleUpdateCoreDetails = async () => {
    ...
    const visibleFields = doctypeFields.filter(f => f.hidden !== 1 && !isSignedDocumentField(f) && f.fieldname !== 'naming_series' && f.fieldname !== 'amended_from' && f.fieldname !== 'workflow_state');
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', flexWrap: 'wrap', gap: '12px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                    ...
                    {!isEditingDetails && <OnboardingQuotationAction
                      key={selectedCase.name}
                      name={selectedCase.name}
                      approved={(workflowState?.current_state || selectedCase.workflow_state) === 'Approved' && selectedCase.docstatus !== 2}
                    ...
                    {activeDetailTab === 'documents' && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1, minHeight: 0 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', flex: 1, overflowY: 'auto', paddingRight: '6px', minHeight: 0 }}>
                          ...
                          <OnboardingSignedDocuments
                            key={selectedCase.name}
                            value={selectedCase[signedDocumentFieldname] || ''}
                            baseUrl={erpnextConfig?.url}
                            upload={file => uploadFileToERPNext(file, erpnextConfig, getCsrfToken)}
                            save={async fileUrl => {
                              ...
                              const res = await fetch(`${erpnextConfig.url}/api/resource/Tenant Onboarding/${encodeURIComponent(caseName)}`, {
                                ...
                                credentials: 'include',
                                headers: getAuthHeaders({ 'Content-Type': 'application/json' }),
                                body: JSON.stringify({ [signedDocumentFieldname]: fileUrl })
                              });
                              if (!res.ok) throw new Error(await extractErrorMessage(res));
                              setSelectedCase(prev => prev?.name === caseName ? { ...prev, [signedDocumentFieldname]: fileUrl } : prev);
                              setOnboardings(prev => prev.map(item => item.name === caseName ? { ...item, [signedDocumentFieldname]: fileUrl } : item));
                            }}
                            preview={url => { setPreviewDocUrl(url); setPreviewDocTitle('Signed Documents'); }}
                          />
                        </div>
